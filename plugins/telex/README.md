...
Code. Install the runtime with `npm install -g @sojaner/telex`, then install this plugin from
`plugins/telex` with the host's plugin installer. In each project, run `telex add` once and
`telex project <name>` to select its bot.

For local Claude Code development, load it with `claude --plugin-dir ./plugins/telex`. Codex
plugins can be installed from a local marketplace; follow the host's plugin browser or marketplace
flow for the checkout you are using. Codex requires you to review and trust the plugin's hooks
before they run.
